...
## Fixture Strategy
Define shared fixtures in `tests/conftest.py` for reuse across test files. Example fixtures:
```python
import pytest
import sys
import os
...
def auth_client(client):
    ...
    return client
```
Adapt fixtures to the actual Spendly API as it exists — adjust based on what fields the register/login forms actually require.
...
- Do NOT read source files for test logic — tests must be spec-based only
